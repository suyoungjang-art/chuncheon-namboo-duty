...
- 검색 시 달력에서 해당 근무일 하이라이트 + 자동 스크롤 기능 추가
- 상단 "당직 공지사항"란 추가 — 관리자 인증번호(기본 `5600`)로 PDF 공지문을 올리면 누구나 다운로드 가능 (Supabase Storage `notices` 버킷 사용, 확인 완료)
- 날짜 칸(이름)을 클릭하면 뜨는 창 맨 아래에 "📝 특이사항" 기록란 추가 — 아무나 자유롭게 적고 저장 가능, 적어두면 달력에 📝 표시가 뜨고 모두에게 공유됨
- 화면 맨 하단 "근무 변경 이력"란은 없애고, 적어둔 특이사항이 날짜순으로 나열되는 "일자별 특이사항"란으로 교체
