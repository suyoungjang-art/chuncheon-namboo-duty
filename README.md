...
- 전체 당직순번 관리·공휴일 관리·이 달 초기화·인증번호 변경 4개 기능을 "🔐 관리자 모드" 버튼 하나로 통합 — 인증번호는 한 번만 입력하면 4개 기능을 모두 이용 가능
- 새로고침(또는 다른 사람의 변경으로 인한 자동 새로고침) 후에도 마지막으로 보던 달을 그대로 유지 (예: 9월을 보던 중 새로고침해도 8월로 안 돌아감) — 각자 브라우저에 기억되는 방식
- 당직 근무시간을 08:20~17:20 → **08:10~17:10**으로 변경 (모든 달에 자동 반영, 확인 완료)
- "다른 날짜와 맞바꾸기" 아래에 "당직근무자 변경"란 추가 — 맞바꿀 상대가 없어도 그 날 담당자 이름만 바로 입력해서 바꿀 수 있음. 이 달에 이미 쓰인 이름을 다시 써도(중복 배정) 제한 없음
- **사용법 튜토리얼 페이지 추가**: https://suyoungjang-art.github.io/chuncheon-namboo-duty/guide.html — 기능마다 직접 눌러보고 따라 할 수 있는 연습 화면이 들어 있음 (실제 자료에는 영향 없음). 메인 화면 검색창 옆 "📖 사용법" 버튼으로도 바로 이동 가능
- 당직 근무시간(08:10~17:10) 표시를 화면에서 완전히 제거 — 메인 앱·튜토리얼 모두 반영, 기존에 저장된 달도 다시 열면 자동으로 정리됨
